...
myConcat([], L, L).
myConcat([Prim|Rest], L2, [Prim|L]) :- myConcat(Rest, L2, L).
...
myReverse([], []).
myReverse([Prim|Rest], RevList):- myReverse(Rest, Aux),
                                  myConcat(Aux, [Prim], RevList).
...
myReverseAcc([Prim|Rest], Acc, RevList) :- myReverseAcc(Rest, [Prim|Acc], RevList).
...
align(L, 0) :- L.
align([_|Rest], N) :- align(Rest, N - 1).

extract_aux(_, 0, Range) :- Range.
extract_aux([Prim|Rest], N, Range) :- extract_aux(Rest, N - 1, [Prim|Range]).

extract(L, Start, End, Range) :- extract_aux(align(L, Start), End, Range).
...
factorial(0, 1).
factorial(N, F) :- X is N - 1, factorial(X, Y), F is N * Y.
...
sublista([], []).
sublista([Prim|Rest1], [Prim|Rest2]) :- sublista(Rest1, Rest2).
sublista([_|Rest1], Rest2) :- sublista(Rest1, Rest2).
...
sameTree(X, Y) :- arc(X, Y); arc(Y, X);
                  isRoot(Z), descendantOf(X, Z), descendantOf(Y, Z).
...
drum(X, Y, [X, Y]).
%:- arc(X, Y); arc(Y, X).
drum(X, Y, [X, Prim|Rest]) :- (arc(X, Prim); arc(Prim, X)), drum(Prim, Y, [Prim|Rest]).
drum(X, Y, [_|Rest]) :- drum(X, Y, Rest).
