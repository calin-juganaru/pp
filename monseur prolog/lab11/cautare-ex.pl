...
safe(_, []) :- true.
safe(X1/Y1/S1, [X2/Y2/S2 | Others]) :- ((S1 == S2, X1 \= X2, Y1 \= Y2); S1 \= S2), safe(X1/Y1/S1, Others).
...
%boat(0, _).
boat(M, C) :- between(0, 2, M), between(0, 2, C), M + C =< 2, M + C > 0, (M >= C; M == 0).
...
safeMisionari(M, C) :- once(M >= C; M == 0).
...
% S1.
next_state(_, est/ME1/CE1/MV1/CV1, vest/ME2/CE2/MV2/CV2)
    :- boat(M, C), M =< ME1, C =< CE1,
        ME2 is ME1 - M, CE2 is CE1 - C, MV2 is MV1 + M, CV2 is CV1 + C,
        safeMisionari(ME2, CE2), safeMisionari(MV2, CV2).


next_state(_, vest/ME1/CE1/MV1/CV1, est/ME2/CE2/MV2/CV2)
...
preorder(isLeaf(X), Parc) :- [X|Parc].
preorder(N, [X]) :- arc(N, X).
preorder(N, [X|Parc]) :- arc(N, X), preorder(X, Parc).
