...
arithmetic x0 r = iter2 x0 r
    where iter2 x y = x : iter2 (x + y)  y

geometric :: Num a => a -> a -> [a]
geometric x0 r = iter3 x0 r
    where iter3 x y = x : iter3 (x * y) y
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms seed = [charToWord (chr (fromIntegral i)) | i <- iterr seed (mkStdGen seed)]
    where iterr z g = ((fst (next g)) `mod` 256) : iterr z (snd (next g))
{-
    4. (1p)
...
tableToFunc x y = snd (head (let find = y in filter (\x -> fst(x) == find) x))
...
intToAlpha c = chr ((c `mod` 26) + 97)

genRotTable :: Int -> [(Char, Char)]
genRotTable o = [(intToAlpha i, intToAlpha (i + o)) | i <- [78..103]]
...
normalize x y = (let l = (length x) - (length y) in
    if (l > 0)
        then (normalize x (y ++ (take (l) y)))
        else
            if (l < 0)
                then (normalize (x ++ (take (-l) x)) y)
                else (x, y))
...
xorStrings x y = iter x y
    where iter x y = if (null x) || (null y) then [] else
            (chr (xorNumbers (ord (head x)) (ord (head y)))) : iter (tail x) (tail y)
...
rollingXor x y = iter (fst (normalize x y)) (snd (normalize x y))
    where iter x y = if (null x) || (null y) then [] else
            (chr (xorNumbers (ord (head x)) (ord (head y)))) : iter (tail x) (tail y)
