...
data Node s a = NewNode s a (Node s a) Int [Node s a]
              | Root s [Node s a]
              | Leaf s a

makeNode :: (a, s) -> Node s a
makeNode (x, y) = Leaf y x

fromSuccessors :: [(a, s)] -> [Node s a]
fromSuccessors list = map makeNode list
...
nodeState (RootNode state _) = state
nodeState (NewNode state _ _ _ _) = state
...
createStateSpaceAux :: (ProblemState s a) => s -> a -> Int -> Node s a -> Node s a
createStateSpaceAux (NewNode state action parent depth kids)
    = if (isGoal state)
        then Leaf state action parent depth
        else NewNode state action parent depth (map (createStateSpaceAux) []

createStateSpace :: (ProblemState s a) => s -> Node s a
createStateSpace state = RootNode state []
