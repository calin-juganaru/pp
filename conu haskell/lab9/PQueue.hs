...
class (Ord a) => PQueue pq a where
    ...
    fromList [] = empty
    fromList (x : y) = insert x (fromList y)

    toList :: pq a -> [(Prio, a)]
    toList empty = []
    toList q = (maybeToList (top q)) ++ (toList (pop q))
...
instance (Ord a) => PQueue ListPQ a where
  ...
  insert elem (LPQ (x : y))
    | (fst elem) > (fst x) = LPQ (elem:x:y)
    | otherwise = fromList ([x] ++ (toList (insert elem (LPQ y))))
...
merge (Empty x) (Node y v l r) = (Node y v l r)
merge (Node x v l r) (Empty y) = (Node x v l r)
merge (Node x1 v1 l1 r1)  (Node x2 v2 l2 r2 )
  | fst v1 > fst v2 = merge (Node x2 v2 l2 r2)(Node x1 v1 l1 r1)
  | x1 >= x2 = (Node (x2 + 1) v1 l1 (merge r1 (Node x2 v2 l2 r2)))
  | otherwise = (Node (x1 + 1) v1 (merge r1 (Node x2 v2 l2 r2)) l1)

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Node x v (Empty l) (Empty r)) = [v]
inorder (Node x v l (Empty r)) = (inorder l) ++ [v]
inorder (Node x v (Empty l) r) = [v] ++ (inorder r)
inorder (Node x v l r) = (inorder l) ++ [v] ++ (inorder r)
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    isEmpty pq = size pq == 0

    insert elem pq = merge (Node 1 elem empty empty) pq

    top (Empty x) = Nothing
    top (Node x v l r) = Just v

    pop (Node x v l r) = merge l r
