...
--unzip_aux :: ([(a, b)], [c])  -> ([a], [b])
--unzip_aux (x, y) = if length(x) == 1 then (fst(head(x)) : [], snd(head(x)) : []) else y ++ (unzip2(tail(x)))
...
-}
divs :: Int -> [Int]
divs n = [i | i <- [2..n-1], mod n i == 0]

primes :: Int -> [Int]
primes n = 2 : [i | i <- [3,5..n], length(divs(i)) == 0]
...
setIntersection a b = [i | i <- a, elem i b]

setDiff :: Eq a => [a] -> [a] -> [a]
setDiff a b = [i | i <- a, not(elem i b)]

cartProduct :: [a] -> [b] -> [(a, b)]
cartProduct a b = [(i, j) | i <- a, j <- b]
...
group2 :: Eq a => [a] -> [[a]]
group2 [] = []
group2 x = ((head x) : y) : group2 z where (y, z) = span ((==) (head x)) (tail x)
...
-}
count x = length [i | i <- (snd x), i == (fst x)]

nrOcc :: Ord a => [a] -> [(a, Int)]
nrOcc l = [(head(i), length i) | i <- group (sort l)]
...
isIsogram x = (x == [i | i <- x, ((isLetter i) && (count (i, x) == 1)) || (not (isLetter i))])
...
pairaux (x:xs) st
  | (null xs) = (null st)
  | x == '(' || x == '[' || x == '{' = pairaux xs (x:st)
  | (not (null st)) && (validPair x (head st)) = pairaux xs (tail st)
  | otherwise = False

arePaired :: String -> Bool
arePaired xs = pairaux xs []
