...
(define B (list 2 3 4 5 6 7 8 9 10))

(define (how-many-b nr Bases)
  (if (empty? Bases) 0
      (if (palindrome? (num->base nr (car Bases)))
          (+ 1 (how-many-b nr (cdr Bases)))
          (how-many-b nr (cdr Bases)))))

(define (first-b-pal start b)
  (if (<= b (how-many-b (+ 1 start) B))
      (+ 1 start)
