...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((x (get-line-segment)) (y (get-end-point x)) (z (get-start-point x))
        (u (- (car y) (car z))) (v (- (cdr y) (cdr z))))
            (sqrt (+ (* u u) (* v v)))))
...
(define (distance p1 p2)
      (let ([func (lambda (x) (compute-length (lambda () x) get-start-point get-end-point))])
        (func (cons p1 p2))))
...
(define (num-concat x y)
  (let ((+ string-append))
    (let ((a (number->string x)))
      (let ((b (number->string y)))
        (string->number (+ a b))))));; Nu stergeți această linie.
...
(define (compute-perimeter points)
  (let perim ((puncte (cons (car (reverse points)) points)))
    (if (= 1 (length puncte))
      0
      (let ((x (car puncte)))
        (let ((y (car (cdr puncte))))
          (+ (distance x y) (perim (cdr puncte))))))))
...
(define (3-sequence-max numbers separator)
  (let separa ((n (append numbers (list separator))) (s separator) (max 0) (sum 0))
    (if (empty? n)
        max
        (if (= (car n) s)
            (if (> sum max)
                (separa (cdr n) s sum 0)
                (separa (cdr n) s max 0))
            (separa (cdr n) s max (+ sum (car n)))))))
...
(define (list-num-concat numbers)
  (if (= (length numbers) 1)
      (car numbers)
      (num-concat (car numbers) (list-num-concat (cdr numbers)))))
...
(define (find-all-suffixes number)
  (let nume (( x (number->list number)))
    (if (= (length x) 0)
        null
        (cons (list-num-concat x) (nume (cdr x))))))
...
(define (run initial-state final-state next)
  (reverse (let iter ((x initial-state) (result '()))
    (if (= x final-state)
        (cons x result)
        (iter (next x) (cons x result))))))
...
(define (generate-number k x)
  (list-num-concat (run x (+ x (* k (- k 1))) (lambda (y) (+ y k)))))
